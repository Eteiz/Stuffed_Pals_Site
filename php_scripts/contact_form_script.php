...
if (empty($firstname)) {
    $json['status'] = 0;
    $json['msg'] = "Please provide a valid first name.";
} else if (empty($lastname)) {
    $json['status'] = 0;
    $json['msg'] = "Please provide a valid last name.";
} else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $json['status'] = 0;
    $json['msg'] = "Please provide a valid email address.";
} else if (empty($message)) {
    $json['status'] = 0;
    $json['msg'] = "Please provide a not empty message.";
} else {
    ...
    if (mail("AartaMambroziak@outlook.com", "Stuffed Pals Contact Form", $message_body, $headers)) {
        $json['status'] = 1;
        $json['msg'] = "Mail successfully sent";
    } else {
        $json['status'] = 0;
        $json['msg'] = "There was a problem sending the email.";
    }
}
